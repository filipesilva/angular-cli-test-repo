...
@Injectable()
export class Project9Mod9Service extends BaseService {
  ...
  post(item: Project9Mod9): Observable<NotificationResult> {
  ...
  put(id: number, item: Project9Mod9): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project9Mod9): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project9Mod9> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project9Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project9Mod9[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project9Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
