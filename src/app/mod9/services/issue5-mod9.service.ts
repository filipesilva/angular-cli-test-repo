...
@Injectable()
export class Issue5Mod9Service extends BaseService {
  ...
  getById(id: number): Observable<Issue5Mod9> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue5Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue5Mod9[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue5Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
