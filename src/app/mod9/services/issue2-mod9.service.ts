...
@Injectable()
export class Issue2Mod9Service extends BaseService {
  ...
  getById(id: number): Observable<Issue2Mod9> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue2Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue2Mod9[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue2Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
