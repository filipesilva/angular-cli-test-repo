...
@Injectable()
export class Severity7Mod9Service extends BaseService {
  ...
  post(item: Severity7Mod9): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity7Mod9): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity7Mod9): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity7Mod9> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity7Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity7Mod9[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity7Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
