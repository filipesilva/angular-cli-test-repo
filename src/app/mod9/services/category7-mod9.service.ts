...
@Injectable()
export class Category7Mod9Service extends BaseService {
  ...
  post(item: Category7Mod9): Observable<NotificationResult> {
  ...
  put(id: number, item: Category7Mod9): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category7Mod9): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category7Mod9> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category7Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category7Mod9[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category7Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category7Mod9[]> {
