...
@Injectable()
export class Status6Mod9Service extends BaseService {
  ...
  post(item: Status6Mod9): Observable<NotificationResult> {
  ...
  put(id: number, item: Status6Mod9): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status6Mod9): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status6Mod9> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status6Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status6Mod9[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status6Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
