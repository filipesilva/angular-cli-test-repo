...
@Injectable()
export class Category4Mod9Service extends BaseService {
  ...
  post(item: Category4Mod9): Observable<NotificationResult> {
  ...
  put(id: number, item: Category4Mod9): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category4Mod9): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category4Mod9> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category4Mod9>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category4Mod9[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category4Mod9[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category4Mod9[]> {
