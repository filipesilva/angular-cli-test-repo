...
@Injectable()
export class Project9Mod8Service extends BaseService {
  ...
  post(item: Project9Mod8): Observable<NotificationResult> {
  ...
  put(id: number, item: Project9Mod8): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project9Mod8): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project9Mod8> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project9Mod8>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project9Mod8[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project9Mod8[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
