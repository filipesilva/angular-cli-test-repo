...
@Injectable()
export class Project2Mod8Service extends BaseService {
  ...
  post(item: Project2Mod8): Observable<NotificationResult> {
  ...
  put(id: number, item: Project2Mod8): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project2Mod8): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project2Mod8> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project2Mod8>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project2Mod8[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project2Mod8[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
