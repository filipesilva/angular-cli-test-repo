...
@Injectable()
export class Issue7Mod8Service extends BaseService {
  ...
  getById(id: number): Observable<Issue7Mod8> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue7Mod8>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue7Mod8[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue7Mod8[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
