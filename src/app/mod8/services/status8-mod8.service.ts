...
@Injectable()
export class Status8Mod8Service extends BaseService {
  ...
  post(item: Status8Mod8): Observable<NotificationResult> {
  ...
  put(id: number, item: Status8Mod8): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status8Mod8): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status8Mod8> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status8Mod8>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status8Mod8[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status8Mod8[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
