...
@Injectable()
export class Issue1Mod6Service extends BaseService {
  ...
  getById(id: number): Observable<Issue1Mod6> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue1Mod6>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue1Mod6[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue1Mod6[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
