...
@Injectable()
export class Issue6Mod6Service extends BaseService {
  ...
  getById(id: number): Observable<Issue6Mod6> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue6Mod6>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue6Mod6[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue6Mod6[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
