...
@Injectable()
export class Project6Mod6Service extends BaseService {
  ...
  post(item: Project6Mod6): Observable<NotificationResult> {
  ...
  put(id: number, item: Project6Mod6): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project6Mod6): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project6Mod6> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project6Mod6>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project6Mod6[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project6Mod6[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
