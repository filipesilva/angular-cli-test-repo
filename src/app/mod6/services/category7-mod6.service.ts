...
@Injectable()
export class Category7Mod6Service extends BaseService {
  ...
  post(item: Category7Mod6): Observable<NotificationResult> {
  ...
  put(id: number, item: Category7Mod6): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category7Mod6): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category7Mod6> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category7Mod6>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category7Mod6[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category7Mod6[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category7Mod6[]> {
