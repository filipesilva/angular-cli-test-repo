...
@Injectable()
export class Project3Mod6Service extends BaseService {
  ...
  post(item: Project3Mod6): Observable<NotificationResult> {
  ...
  put(id: number, item: Project3Mod6): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project3Mod6): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project3Mod6> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project3Mod6>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project3Mod6[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project3Mod6[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
