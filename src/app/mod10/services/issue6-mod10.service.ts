...
@Injectable()
export class Issue6Mod10Service extends BaseService {
  ...
  getById(id: number): Observable<Issue6Mod10> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue6Mod10>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue6Mod10[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue6Mod10[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
