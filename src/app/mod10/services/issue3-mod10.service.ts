...
@Injectable()
export class Issue3Mod10Service extends BaseService {
  ...
  getById(id: number): Observable<Issue3Mod10> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue3Mod10>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue3Mod10[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue3Mod10[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
