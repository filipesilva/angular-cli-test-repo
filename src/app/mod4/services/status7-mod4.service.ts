...
@Injectable()
export class Status7Mod4Service extends BaseService {
  ...
  post(item: Status7Mod4): Observable<NotificationResult> {
  ...
  put(id: number, item: Status7Mod4): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status7Mod4): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status7Mod4> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status7Mod4>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status7Mod4[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status7Mod4[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
