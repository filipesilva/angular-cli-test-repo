...
@Injectable()
export class Severity10Mod4Service extends BaseService {
  ...
  post(item: Severity10Mod4): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity10Mod4): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity10Mod4): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity10Mod4> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity10Mod4>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity10Mod4[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity10Mod4[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
