...
@Injectable()
export class Project4Mod4Service extends BaseService {
  ...
  post(item: Project4Mod4): Observable<NotificationResult> {
  ...
  put(id: number, item: Project4Mod4): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project4Mod4): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project4Mod4> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project4Mod4>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project4Mod4[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project4Mod4[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
