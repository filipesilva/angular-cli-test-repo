...
@Injectable()
export class Issue6Mod4Service extends BaseService {
  ...
  getById(id: number): Observable<Issue6Mod4> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue6Mod4>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue6Mod4[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue6Mod4[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
