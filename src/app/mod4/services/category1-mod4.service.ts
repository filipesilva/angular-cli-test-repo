...
@Injectable()
export class Category1Mod4Service extends BaseService {
  ...
  post(item: Category1Mod4): Observable<NotificationResult> {
  ...
  put(id: number, item: Category1Mod4): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category1Mod4): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category1Mod4> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category1Mod4>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category1Mod4[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category1Mod4[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category1Mod4[]> {
