...
@Injectable()
export class Issue8Mod4Service extends BaseService {
  ...
  getById(id: number): Observable<Issue8Mod4> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue8Mod4>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue8Mod4[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue8Mod4[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
