...
@Injectable()
export class Project1Mod7Service extends BaseService {
  ...
  post(item: Project1Mod7): Observable<NotificationResult> {
  ...
  put(id: number, item: Project1Mod7): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project1Mod7): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project1Mod7> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project1Mod7>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project1Mod7[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project1Mod7[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
