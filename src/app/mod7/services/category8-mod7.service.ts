...
@Injectable()
export class Category8Mod7Service extends BaseService {
  ...
  post(item: Category8Mod7): Observable<NotificationResult> {
  ...
  put(id: number, item: Category8Mod7): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category8Mod7): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category8Mod7> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category8Mod7>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category8Mod7[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category8Mod7[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category8Mod7[]> {
