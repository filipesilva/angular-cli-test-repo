...
@Injectable()
export class Status10Mod7Service extends BaseService {
  ...
  post(item: Status10Mod7): Observable<NotificationResult> {
  ...
  put(id: number, item: Status10Mod7): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status10Mod7): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status10Mod7> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status10Mod7>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status10Mod7[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status10Mod7[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
