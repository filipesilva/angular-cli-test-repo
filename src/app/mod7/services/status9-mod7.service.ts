...
@Injectable()
export class Status9Mod7Service extends BaseService {
  ...
  post(item: Status9Mod7): Observable<NotificationResult> {
  ...
  put(id: number, item: Status9Mod7): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status9Mod7): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status9Mod7> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status9Mod7>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status9Mod7[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status9Mod7[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
