...
@Injectable()
export class Issue7Mod7Service extends BaseService {
  ...
  getById(id: number): Observable<Issue7Mod7> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue7Mod7>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue7Mod7[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue7Mod7[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
