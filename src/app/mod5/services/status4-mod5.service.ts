...
@Injectable()
export class Status4Mod5Service extends BaseService {
  ...
  post(item: Status4Mod5): Observable<NotificationResult> {
  ...
  put(id: number, item: Status4Mod5): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status4Mod5): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status4Mod5> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status4Mod5>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status4Mod5[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status4Mod5[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
