...
@Injectable()
export class Issue4Mod5Service extends BaseService {
  ...
  getById(id: number): Observable<Issue4Mod5> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue4Mod5>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue4Mod5[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue4Mod5[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
