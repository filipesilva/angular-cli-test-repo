...
@Injectable()
export class Category6Mod5Service extends BaseService {
  ...
  post(item: Category6Mod5): Observable<NotificationResult> {
  ...
  put(id: number, item: Category6Mod5): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category6Mod5): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category6Mod5> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category6Mod5>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category6Mod5[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category6Mod5[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category6Mod5[]> {
