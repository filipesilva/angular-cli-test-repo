...
@Injectable()
export class Severity3Mod5Service extends BaseService {
  ...
  post(item: Severity3Mod5): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity3Mod5): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity3Mod5): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity3Mod5> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity3Mod5>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity3Mod5[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity3Mod5[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
