...
export class CultureService extends BaseService {
  ...
  post(item: Culture): Observable<NotificationResult> {
    const url = `${this.urlApi}/cg/culture`;
    return this.httpClient.post<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  put(id: string, item: Culture): Observable<NotificationResult> {
    const url = `${this.urlApi}/cg/culture/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Culture): Observable<NotificationResult> {
    if (newRecord) {
      return this.post(item);
    }
    return this.put(item.idCulture, item);
  }
  ...
  getById(id: string): Observable<Culture> {
    const url = `${this.urlApi}/cg/culture/${id}`;
    return this.httpClient.get<Culture>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Culture[]> {
    const url = `${this.urlApi}/cg/culture`;
    return this.httpClient.get<Culture[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getSelectList(): Observable<IOption[]> {
    const url = `${this.urlApi}/cg/culture/selectList`;
    return this.httpClient.get<Culture[]>(url, { headers: this.getAuthHeaders() })
      .map(resp => {
        return resp.map(x => ({ value: x.idCulture.toString(), label: x.cultureName }));
      })
      .catch(err => this.handleError(err));
  }

  page(gridParam: GridParameter): Observable<GridData> {
    const url = `${this.urlApi}/cg/culture/page/${gridParam.urlParams}`;
