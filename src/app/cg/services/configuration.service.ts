...
export class ConfigurationService extends BaseService {
  ...
  post(item: Configuration): Observable<NotificationResult> {
    const url = `${this.urlApi}/cg/configuration`;
    return this.httpClient.post<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  put(id: string, item: Configuration): Observable<NotificationResult> {
    const url = `${this.urlApi}/cg/configuration/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Configuration): Observable<NotificationResult> {
    if (newRecord) {
      return this.post(item);
    }
    return this.put(item.key, item);
  }
  ...
  getById(id: string): Observable<Configuration> {
    const url = `${this.urlApi}/cg/configuration/${id}`;
    return this.httpClient.get<Configuration>(url, { headers: this.getAuthHeaders() })
  ...
  page(gridParam: GridParameter): Observable<GridData> {
    const url = `${this.urlApi}/cg/configuration/page/${gridParam.urlParams}`;
