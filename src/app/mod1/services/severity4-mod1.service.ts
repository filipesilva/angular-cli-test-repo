...
@Injectable()
export class Severity4Mod1Service extends BaseService {
  ...
  post(item: Severity4Mod1): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity4Mod1): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity4Mod1): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity4Mod1> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity4Mod1>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity4Mod1[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity4Mod1[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
