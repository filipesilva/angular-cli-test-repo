...
@Injectable()
export class Category1Mod3Service extends BaseService {
  ...
  post(item: Category1Mod3): Observable<NotificationResult> {
  ...
  put(id: number, item: Category1Mod3): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category1Mod3): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category1Mod3> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category1Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category1Mod3[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category1Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category1Mod3[]> {
