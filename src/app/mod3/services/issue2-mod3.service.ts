...
@Injectable()
export class Issue2Mod3Service extends BaseService {
  ...
  getById(id: number): Observable<Issue2Mod3> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue2Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue2Mod3[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue2Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
