...
@Injectable()
export class Severity7Mod3Service extends BaseService {
  ...
  post(item: Severity7Mod3): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity7Mod3): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity7Mod3): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity7Mod3> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity7Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity7Mod3[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity7Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
