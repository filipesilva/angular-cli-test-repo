...
@Injectable()
export class Project9Mod3Service extends BaseService {
  ...
  post(item: Project9Mod3): Observable<NotificationResult> {
  ...
  put(id: number, item: Project9Mod3): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project9Mod3): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project9Mod3> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project9Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project9Mod3[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project9Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
