...
@Injectable()
export class Status5Mod3Service extends BaseService {
  ...
  post(item: Status5Mod3): Observable<NotificationResult> {
  ...
  put(id: number, item: Status5Mod3): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status5Mod3): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status5Mod3> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status5Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status5Mod3[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status5Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
