...
@Injectable()
export class Project10Mod3Service extends BaseService {
  ...
  post(item: Project10Mod3): Observable<NotificationResult> {
  ...
  put(id: number, item: Project10Mod3): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project10Mod3): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project10Mod3> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project10Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project10Mod3[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project10Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
