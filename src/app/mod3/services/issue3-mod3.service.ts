...
@Injectable()
export class Issue3Mod3Service extends BaseService {
  ...
  getById(id: number): Observable<Issue3Mod3> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue3Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue3Mod3[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue3Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
