...
@Injectable()
export class Severity2Mod3Service extends BaseService {
  ...
  post(item: Severity2Mod3): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity2Mod3): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity2Mod3): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity2Mod3> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity2Mod3>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity2Mod3[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity2Mod3[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
