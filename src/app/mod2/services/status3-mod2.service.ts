...
@Injectable()
export class Status3Mod2Service extends BaseService {
  ...
  post(item: Status3Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Status3Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status3Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status3Mod2> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status3Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status3Mod2[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status3Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
