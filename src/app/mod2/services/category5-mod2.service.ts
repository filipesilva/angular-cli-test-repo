...
@Injectable()
export class Category5Mod2Service extends BaseService {
  ...
  post(item: Category5Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Category5Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category5Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category5Mod2> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category5Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category5Mod2[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category5Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category5Mod2[]> {
