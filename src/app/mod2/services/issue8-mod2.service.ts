...
@Injectable()
export class Issue8Mod2Service extends BaseService {
  ...
  getById(id: number): Observable<Issue8Mod2> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue8Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue8Mod2[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue8Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
