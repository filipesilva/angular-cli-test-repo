...
@Injectable()
export class Severity1Mod2Service extends BaseService {
  ...
  post(item: Severity1Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Severity1Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Severity1Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Severity1Mod2> {
    const url = `${this.urlApi}/mod/severity/${id}`;
    return this.httpClient.get<Severity1Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Severity1Mod2[]> {
    const url = `${this.urlApi}/mod/severity`;
    return this.httpClient.get<Severity1Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
