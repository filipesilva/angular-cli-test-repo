...
@Injectable()
export class Project8Mod2Service extends BaseService {
  ...
  post(item: Project8Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Project8Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project8Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project8Mod2> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project8Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project8Mod2[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project8Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
