...
@Injectable()
export class Issue6Mod2Service extends BaseService {
  ...
  getById(id: number): Observable<Issue6Mod2> {
    const url = `${this.urlApi}/mod/issue/${id}`;
    return this.httpClient.get<Issue6Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Issue6Mod2[]> {
    const url = `${this.urlApi}/mod/issue`;
    return this.httpClient.get<Issue6Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
