...
@Injectable()
export class Status1Mod2Service extends BaseService {
  ...
  post(item: Status1Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Status1Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Status1Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Status1Mod2> {
    const url = `${this.urlApi}/mod/status/${id}`;
    return this.httpClient.get<Status1Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Status1Mod2[]> {
    const url = `${this.urlApi}/mod/status`;
    return this.httpClient.get<Status1Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
