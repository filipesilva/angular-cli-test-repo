...
@Injectable()
export class Category9Mod2Service extends BaseService {
  ...
  post(item: Category9Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Category9Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Category9Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Category9Mod2> {
    const url = `${this.urlApi}/mod/category/${id}`;
    return this.httpClient.get<Category9Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Category9Mod2[]> {
    const url = `${this.urlApi}/mod/category`;
    return this.httpClient.get<Category9Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
  ...
  getRecursive(): Observable<Category9Mod2[]> {
