...
@Injectable()
export class Project7Mod2Service extends BaseService {
  ...
  post(item: Project7Mod2): Observable<NotificationResult> {
  ...
  put(id: number, item: Project7Mod2): Observable<NotificationResult> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.put<NotificationResult>(url, item, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }

  save(newRecord: boolean, item: Project7Mod2): Observable<NotificationResult> {
    if (newRecord) {
  ...
  getById(id: number): Observable<Project7Mod2> {
    const url = `${this.urlApi}/mod/project/${id}`;
    return this.httpClient.get<Project7Mod2>(url, { headers: this.getAuthHeaders() })
      .catch(err => super.handleError(err));
  }

  get(): Observable<Project7Mod2[]> {
    const url = `${this.urlApi}/mod/project`;
    return this.httpClient.get<Project7Mod2[]>(url, { headers: this.getAuthHeaders() })
      .catch(err => this.handleError(err));
  }
